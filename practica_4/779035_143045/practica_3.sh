	...
	then
		...
		case "$1" in
			-a)
				...
				do
					...
					then
						...
						useradd "$nombre" -U -m -k /etc/skel -K UID_MIN=1815 -c "$apodo"
			...
			-s)
				...
				do
					nombre=$(echo "$linea" | cut -d ',' -f 1)
					directorio=$(grep "$nombre" /etc/passwd | cut -d ':' -f 6)
					homeUsuario="${directorio%/*}"
					if tar -cf "$nombre".tar "$homeUsuario"/"$nombre"
					then
						mv "$nombre".tar /extra/backup
